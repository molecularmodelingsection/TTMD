...
def calcRMSD(topology, trajectory):
    ...
    # R = rms.RMSD(u, u, select='nucleicbackbone', ref_frame=0).run() #adodaro
    R = rms.RMSD(u, u, select='(resid 1:307) and name CA ', ref_frame=0).run() #adodaro resid 1:219 or resid 264:342
...
def bsite_rmsd(rmsd_resids, topology, done_temp, stop_range):
    ...
    for temp in done_temp:
        ...
        R = rms.RMSD(u, u, select=f'({selection}) and name CA', ref_frame=0).run() #adodaro

...
class graphs:
    ...
    def intraprotein_df(self):
        ...
        ref_bonds = count_hbonds(self.solvprmtop, self.output['eq2']['dcd'], selection, 'nucleic', 100, 'ref_prot_hbonds')
        
        mp_hbonds = []
        
        for i in self.done_temp:
            mp_hbonds.append([self.solvprmtop, f'../MD/swag_{i}.dcd', selection, 'nucleic', self.stop_range, f'prot_hbonds_{i}'])
            
        temp_hbonds = self.parallelizer.run(mp_hbonds, count_hbonds, '      Calculating Protein DF')
    ...
    def h2oprotein_df(self):
        ...
        temp_hbonds = self.parallelizer.run(mp_hbonds, count_hbonds, '      Calculating H2O-Protein DF')

...
quit'''
    ...
    def dry_trj(self, trajectory, set):
        ...
        if check == False:
            ...
            udry = u.select_atoms('not resname WAT and not resname Na+ and not resname NA and not resname Cl- and not resname CL')
...
def wrap_blocks(first, last, topology, trajectory):
    ...
    with mda.Writer(block_name, u.atoms.n_atoms) as W:
        for ts in u.trajectory[first:last]:
            old_rmsd, new_rmsd = align.alignto(u, ref, select='(resid 1:307) and name CA', weights='mass') #adodaro resid 1:219 or resid 264:342
            W.write(u.atoms)
